...
//---------------------------------------------------------------------------
TStringDynArray get_csv_array_Q(UnicodeString src, int size,
	bool del_q)		//引用符を外す  (default = true)
{
	TStringDynArray ret_array;
	ret_array.set_length(size);

	UnicodeString strbuf = src;
	...
	while (!quit) {
		...
		if (strbuf.IsDelimiter("\"", 1)) {	//先頭が "
			...
			if (!del_q) tmpbuf = "\"" + tmpbuf + "\"";
		}
		else {
			qp = strbuf.Pos(",");
			if (qp) {
				tmpbuf = strbuf.SubString(1, qp-1);
				strbuf = strbuf.Delete(1,qp);
			}
			//最後の項目
			else {
				tmpbuf = strbuf;
...
UnicodeString cv_TabToSpc(UnicodeString s, TCanvas *cv)
{
	UnicodeString ret_str;
	int wd = 0;
	int wd_s = cv->TextWidth(" ");
	for (int i=1; i<=s.Length(); i++) {
		UnicodeString cs = (s[i]=='\t')? StringOfChar(_T(' '), 4 - (wd/wd_s)%4) : s[i];
		ret_str += cs;
		wd += cv->TextWidth(cs);
	}
	return ret_str;
